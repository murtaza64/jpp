...
export default function parseWords() : WordMap {
  let map: WordMap = new Map<string, WordMapEntry>()
  const data = readFileSync("public/words.txt", {encoding: "utf-8"})
  data.split("ーーー")
    .map((s) => s.trim().split("\n"))
    .filter((s) => s.length > 1)
    .forEach((item) => {
      ...
      const peak = item[2].split(" ").findIndex((v) => v[v.length-1] == "^")
      const pitches = item[2].split(" ").map((v) => {
        if (v[v.length-1] == "^")
